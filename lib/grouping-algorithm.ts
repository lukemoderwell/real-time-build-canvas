...
const GROUP_PATTERNS: KeywordPattern[] = [
  {
    keywords: ["auth", "login", "signup", "password", "oauth", "session", "user", "sso", "mfa"],
    groupName: "Authentication",
    color: "#3b82f6",
  },
  {
    keywords: ["payment", "stripe", "checkout", "subscription", "billing", "invoice", "plan", "pricing", "credit card"],
    groupName: "Billing & Payments",
    color: "#8b5cf6",
  },
  {
    keywords: ["database", "postgres", "supabase", "sql", "query", "storage", "data", "schema", "table", "record"],
    groupName: "Data Layer",
    color: "#06b6d4",
  },
  {
    keywords: ["ui", "design", "component", "layout", "style", "theme", "dark mode", "css", "frontend", "ux"],
    groupName: "Design System",
    color: "#ec4899",
  },
  {
    keywords: ["api", "endpoint", "route", "handler", "backend", "server", "function", "middleware", "controller"],
    groupName: "Backend API",
    color: "#10b981",
  },
  {
    keywords: ["email", "notification", "alert", "message", "send", "communication", "sms", "push"],
    groupName: "Notifications",
    color: "#f59e0b",
  },
  {
    keywords: ["admin", "dashboard", "analytics", "chart", "report", "metrics", "stats", "moderation"],
...
export function updateGroups(nodes: NodeData[], currentGroups: NodeGroup[]): NodeGroup[] {
  // Create a map of currently grouped node IDs to avoid moving them
  const groupedNodeIds = new Set<string>()
  currentGroups.forEach((g) => g.nodeIds.forEach((id) => groupedNodeIds.add(id)))
  ...
  const remainingNodes = nodesToGroup.filter((n) => !assignedInThisPass.has(n.id))
  ...
  const stillUngrouped = remainingNodes.filter((n) => !assignedInThisPass.has(n.id))
